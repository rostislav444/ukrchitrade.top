...
from apps.catalogue.serilaizers import ProductSearchSerializer
...
class SearchView(View):
    ...
    def match_products(self, param):
        ...
        if param:
            for product in list(Product.objects.all()):
                ...
                if ratio >= 65:
                    prod = ProductSearchSerializer(product).data
                    prod['ratio'] = ratio
    ...
    def post(self, request):
        ...
        render_to_string('shop/search/search__popup_results.html', {'products' :  products})
        return JsonResponse({'html' : str(products)})
